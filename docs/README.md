...
pr_title        No      🛡️ Automated Pull Request        Title of the derived PR
...
Variables:
        •       ATTACHED_FILE_PATH (file basename only)
        •       PR_NUMBER
        •       BRANCH_CONVENTION
...
Variables:
        ...
        •       PR_NUMBER
        •       BRANCH_CONVENTION
        •       PR_URL (URL of the created/reused derived PR)
...
Recommended conventions
        ...
        •       Generated files:
        •       .reports/* (or an equivalent folder, versioned/ignored per your strategy)
        •       Error logs:
        •       .errors/ops-publisher.log

Operational notes
        •       If there are no changes to the file (git diff --cached --quiet), the Action will not create a commit or a new PR.
        •       The derived PR is always opened with base = source branch of the source PR, forming an integration chain.
        •       To centralize failures, use `errors` and collect this file as a workflow artifact when necessary.
